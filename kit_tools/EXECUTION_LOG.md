...
### US-001: Add SUBPROCESS node type and config (Attempt 1) — PASS
- Completed: 2026-05-15T15:07:53Z
- Verified by: independent verifier session
- Learnings: NodeType retry guard uses an allowlist (AGENT, AGENT_POOL only), so SUBPROCESS is automatically rejected without any additional code — no separate exclusion needed.; VALID_CONFIGS dict in test_schema.py must be updated whenever a new NodeType is added — it is used by parametrized tests that iterate all NodeType values.; TestNodeType had a test named test_all_eight_values with a hardcoded set — rename and update when adding new types.; Verifier note: Clean, well-scoped implementation. The SubProcessNodeConfig model matches the spec exactly: process_id/output_key required, mappings default to empty, max_depth bounded 1-20 with default 5. Tests are comprehensive — covering minimal config, full config, required-field validation, both max_depth boundaries, CONFIG_MAP discriminator parsing, and an end-to-end YAML parse via ProcessDefinition. The retry-rejection requirement is satisfied via the existing inclusion-list pattern rather than the exclusion-list approach suggested in the hint, but the runtime behavior is identical and the test parametrization confirms SUBPROCESS is rejected. No scope creep: only schema.py and test_schema.py touched, which is exactly what this US-001 schema-only story should change.
- Committed: feat(subprocess-schema): US-001 - Add SUBPROCESS node type and config

### US-002: Add subprocess event types and escalation trigger (Attempt 1) — PASS
- Completed: 2026-05-15T15:11:38Z
- Verified by: independent verifier session
- Learnings: test_events.py had a stale test name (test_all_18_event_types_defined) with an already-updated count of 19 — renamed to test_all_22_event_types_defined after adding 3 subprocess events.; SubProcessNodeConfig must be imported into orchestrator.py to use in assert isinstance() inside the stub handler.; Pyright diagnostics on EdgeDefinition(from_node=..., to_node=...) calls throughout test files are pre-existing issues unrelated to this story.; Verifier note: Implementation is minimal and tightly scoped to the hints in the story. The stub handler asserts the config type before raising, which is a nice defensive touch — guarantees the node config schema is wired correctly even though execution is stubbed. The integration test goes through the real orchestrator.start_run / execute_run path rather than calling _handle_subprocess directly, which is the right level for a stub-handler smoke test. The test_events.py enum-count test was updated from 18→22 (matches actual count of 22 entries). No scope creep, no dead code, conventions adhered to.
- Committed: feat(subprocess-schema): US-002 - Add subprocess event types and escalation trigger

### US-003: Add parent/child run relationship to storage (Attempt 1) — PASS
- Completed: 2026-05-15T15:18:00Z
- Verified by: independent verifier session
- Learnings: SQLite migration pattern: add new columns to _SCHEMA_SQL CREATE TABLE (for new DBs), then use try/except OperationalError ALTER TABLE in initialize() (for existing DBs). Index must be created AFTER ALTER TABLE, not in _SCHEMA_SQL, because CREATE INDEX on a not-yet-existing column would fail for old DBs.; PostgreSQL migration pattern: ALTER TABLE runs ADD COLUMN IF NOT EXISTS is idempotent and safe to run on every initialize(). CREATE INDEX IF NOT EXISTS is similarly idempotent.; The keyword-only parameter placement (*) after process_version keeps backward compatibility — existing callers that pass process_version positionally are unaffected.; Migration test uses tmp_path fixture + aiosqlite to pre-create an old-schema DB without parent columns, then calls initialize() and verifies the columns are added and usable.; Verifier note: Clean implementation. RunRecord, abstract interface, and both backends consistently updated. Migration path is correct: ALTER TABLE wrapped in try/except for SQLite (the documented Roots pattern for first ALTER TABLE migration) and ADD COLUMN IF NOT EXISTS for PostgreSQL. Index added in both backends for efficient get_child_runs lookup. New keyword-only params keep all ~80 existing create_run callsites working without modification — including orchestrator.start_run, which is appropriate (top-level runs have no parent). get_child_runs hydrates RunRecord fully including the new parent fields. Postgres-side tests are not exercised against a live DB (test_postgres fixtures skipped without DSN), but the postgres code path mirrors the sqlite one and is covered by the shared abstract contract in tests/test_storage.py if run with the Postgres fixture.
- Committed: feat(subprocess-schema): US-003 - Add parent/child run relationship to storage

### US-004: Validate subprocess references (Attempt 1) — PASS
- Completed: 2026-05-15T15:24:59Z
- Verified by: independent verifier session
